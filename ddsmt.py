...
g_num_run_limit = 0 # counter of z3 runs
g_time_limit = 15   # time limit for each run. TODO: add an option for setting this
...
def _legendtest():
    global g_num_run_limit
    global g_args, g_ntests, g_testtime
    g_ntests += 1
    #print(g_args.line)
    use_line = False
    if g_args.line > 0: use_line = True # TODO: use line number
    ret = False
    proc = None
    ...
    try:
      for line in iter(proc.stdout.readline, ''):
        ...
        if not use_line:
          if decode_line.startswith("ASSE") or decode_line.startswith("Fatal"):
            ret = True; break
        else:
          if "Line" in decode_line:
            #print("Find Line Info")
            if str(g_args.line) in decode_line:
              print("Find Exact Line")
              ret = True; break
          elif ".cpp:" in decode_line or ".h:" in decode_line:  # for cvc4
            if str(g_args.line) in decode_line:
              print("Find Exact Line") 
              ret = True; break   
    except Exception as e:
      print(e)
      print("exception when running z3, willl return false")
      #return ret
    proc.stdout.close()
    procstat = proc.wait()
    timer.cancel()
    g_num_run_limit += 1
    stat = 'fail'
    if ret == True: stat = 'success'
    print(g_num_run_limit, "-th run: ", stat)
    return ret
...
def _substitute (subst_fun, substlist, superset, randomized,
    ...
    while gran > 0:
        ...
        for subset in subsets:
            ...
            start = time.time()
            cur_test_res = False
            if g_args.mode == 1: cur_test_res = _legendtest()
            else: cur_test_res =  _test()
            #if _test():
            if cur_test_res:
...
def ddsmt_main ():
    ...
    while nsubst_round:
        ...
        if g_args.cmdfirst and nrounds == 1: # is this correct?
          print('Will try to eliminate cmd first')
          nsubst = _substitute_cmds (g_args.bfs, g_args.randomized,
                                      lambda x: x.is_set_option())
          # when should be stop?
        else:
          if nrounds > 1:
           nsubst = _substitute_cmds (g_args.bfs, g_args.randomized)
        ...
        if g_args.cmdfirst and nrounds == 1: break
        if g_num_run_limit >= 1000: break
...
if __name__ == "__main__":
    try:
        ...
        aparser.add_argument ("-m", action="count", dest="mode", default=0,
                              help="set the mode")

        aparser.add_argument ("-cmd", action="store_true", dest="cmdfirst",\
                              default=False,
                              help="only eliminate cmds")
    
        aparser.add_argument ("-l",  dest="line", type=int, default=0,
                              help="line number of the bug")
