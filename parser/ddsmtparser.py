...
KIND_STR_INTTOSTR = "int.to.str"
KIND_STR_FROM_CODE = "str.from_code"
KIND_STR_CONCAT   = "str.++"
...
KIND_SETMINUS           = "setminus"
KIND_SETUNION           = "union"
KIND_SETINTERSECTION    = "intersection"
KIND_SETSUBSET          = "subset"
KIND_SETCOMPLEMENT      = "complement"
KIND_SETCARD            = "card"
...
KIND_ASSERT        = "assert"
KIND_ASSERTSOFT    = "assert-soft"
KIND_CHECKSAT      = "check-sat"
KIND_RESETASSE      = "reset-assertions"
KIND_CHECKSATUSING = "check-sat-using" 
KIND_ELIMINATEQUANT = "eliminate-quantifiers"
KIND_CHECKSATASSUME = "check-sat-assuming"
KIND_GETCONSEQUENCE = "get-consequences"
...
KIND_MINIMIZE  = "minimize"
KIND_MAXIMIZE  = "maximize"
...
class SMTFunAppNode (SMTNode):
    ...
    def is_setminus(self):
        return self.kind == KIND_SETMINUS
...
class SMTCmdNode:
    ...
    def dump (self, outfile, lead = ""):
        ...
        elif self.kind == KIND_ASSERTSOFT: # TODO: corrent?
            outfile.write("({}".format(self.kind))
            assert (len(self.children) == 1)
            self.children[0].dump(outfile)
            outfile.write(")\n")
    ...
    def is_set_option(self):
        return self.kind == KIND_SETOPT

    def is_assert_soft (self):
        return self.kind == KIND_ASSERTSOFT
    ...
    def is_minimize(self):
        return self.kind == KIND_MINIMIZE

    def is_maximize(self):
        return self.kind == KIND_MAXIMIZE
...
class SMTFormula:
    ...
    def check_funApp (self, fun, kind, children):
        ...
        for c in children:
            #if str(c) in ('RNA', 'RTP', 'RNE', 'RTN', 'RTZ'): continue
            if not c.sort:
                #if c.kind not in (KIND_SETMINUS, KIND_SETUNION, KIND_SETINTERSECTION,
                #     KIND_SETSUBSET, KIND_SETCOMPLEMENT,KIND_SETCARD):
                assert (c.kind == KIND_FUN)  
                raise DDSMTParseCheckException (
                        "function '{!s}' undeclared".format(c))
        # number of args check
        if ((len(children) != 1 and
                 kind in (KIND_ABS, KIND_SIN, KIND_BVNEG, KIND_BVNOT, KIND_EXTR, KIND_ISI,
                          KIND_NOT, KIND_NEG,   KIND_TOI,   KIND_TOR,  KIND_REP,
                          KIND_ROL, KIND_ROR,   KIND_SEXT,  KIND_ZEXT,
                          KIND_STR_LEN, KIND_STR_INTTOSTR, KIND_STR_FROM_CODE)) or
            (len(children) != 2 and
                 kind in (KIND_BVADD,  KIND_BVAND,  KIND_BVASHR, KIND_BVCOMP,
                          ...
                          KIND_SELECT, KIND_STR_SUFFIXOF, KIND_STR_PREFIXOF,
                          KIND_STR_LESSEQ, KIND_STR_LESS)) or
            (len(children) != 3 and
        ...
        if kind in (KIND_AND, KIND_IMPL, KIND_NOT, KIND_OR, KIND_XOR):
            for c in children:
                if not c.sort == sortbool:
                    # TODO: this is a dirty fix for Set
                    # print(str(c))
                    if not (str(c).startswith("(Set") or str(c).startswith("(subset")):
                      raise DDSMTParseCheckException (
                        "'{!s}' expects sort 'Bool' as argument(s), " \
                        "found '{}'".format(fun, c.sort))
        # args Int check
        elif kind in (KIND_ABS, KIND_STR_INTTOSTR, KIND_STR_FROM_CODE, KIND_DIV, KIND_MOD, KIND_TOR):
            for c in children:
                #if not c.sort == sortint:
                if not c.sort == sortint and kind != KIND_ABS:
                    raise DDSMTParseCheckException (
                        "'{!s}' expects sort 'Int' as argument(s), " \
                        "found '{}'".format(fun, c.sort))
        # args Real check
        elif kind in (KIND_RDIV, KIND_SIN,  KIND_ISI, KIND_TOI):
            for c in children:
        ...
        # args Int or Real check
            ...
            if c0.sort not in (sortint, sortreal):
                  # TODO; fix paring erro in '(> (card setxx) 2)'. The line below is a dirty fix
                  if not str(c0).startswith("(card"):  
                    raise DDSMTParseCheckException (
        ...
        # Set
        elif kind in (KIND_SETMINUS, KIND_SETUNION, KIND_SETINTERSECTION, 
                     KIND_SETSUBSET, KIND_SETCOMPLEMENT,KIND_SETCARD):
            #print(kind)
            if 0 > 1: raise DDSMTParseCheckException ("Set grammar check failure")             
        ...
        elif kind in (KIND_FPEQ, KIND_FPLE, KIND_FPLT, KIND_FPGE, KIND_FPGT,
                     KIND_FPABS, KIND_FPSQRT, KIND_FPADD, KIND_FPSUB, KIND_FPMUL, KIND_FPDIV):
             if 0 > 1: 
                raise DDSMTParseCheckException ("FP function error")
        # not predefined
        else:
    ...
    def funApp2sort (self, fun, kind, children):
        ...
        # sort Real
        elif kind in (KIND_RDIV, KIND_TOR, KIND_SIN):
            return self.sortNode("Real")
        # sort String
        elif kind in (KIND_STR_CONCAT, KIND_STR_SUBSTR, KIND_STR_REPLACE, KIND_STR_INTTOSTR, KIND_STR_FROM_CODE):
        ...
        elif kind in (KIND_SETMINUS, KIND_SETUNION, KIND_SETINTERSECTION, KIND_SETSUBSET, KIND_SETCOMPLEMENT,KIND_SETCARD):
            return children[0].sort
        ...
        try:
            if len(t) == 1:
                ...
                if t_ident[0] == SMTParser.IDXED:
                    ...
                    assert (len(t_ident[2]) == 1)  # Is this OK for fP?
