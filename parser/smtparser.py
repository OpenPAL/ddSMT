...
class SMTParser:
    ...
    POP       = "pop"
    ASSERT    = "assert"
    ASSERTSOFT = "assert-soft"
    CHECKSAT  = "check-sat"
    RESETASSE = "reset-assertions"
    CHECKSATUSING  = "check-sat-using"
    ELIMINATEQUANT = 'eliminate-quantifiers'
    CHECKSATASSUME = "check-sat-assuming"
    GETCONSEQUENCE = "get-consequences"
    ...
    def __command (self):
        ...
        if self.la == SMTParser.SETLOGIC:
        ...
        elif self.la == SMTParser.ASSERTSOFT: # TODO; corrent?
            self.__scan()
            tokens.append(self.term.parse_action(self.__term()))
        elif self.la == SMTParser.CHECKSAT:
            self.__scan()
        elif self.la == SMTParser.RESETASSE:
            self.__scan()
        elif self.la == SMTParser.CHECKSATUSING:
            self.__scan()
            self.__check_lpar()
            tokens.append([])
            while self.la and self.la != SMTParser.RPAR:
        ...
        elif self.la == SMTParser.ELIMINATEQUANT:
            self.__scan()
            self.__check_lpar()
            tokens.append([])
            while self.la and self.la != SMTParser.RPAR:
            ...
            self.__check_rpar()
        #ELIMINATEQUANT
        elif self.la == SMTParser.CHECKSATASSUME:
            self.__scan()
            self.__check_lpar()
            tokens.append([])
            while self.la and self.la != SMTParser.RPAR:
            ...
            if not tokens[-1]:
                raise SMTParseException ("term expected", self)
            self.__check_rpar()
        elif self.la == SMTParser.GETCONSEQUENCE: # TODO: how??
            self.__scan()
            #self.__check_lpar()
            tokens.append([])
            while self.la and self.la != SMTParser.RPAR:
        ...
        elif self.la == SMTParser.MINIMIZE:
            self.__scan()
            tokens.append(self.term.parse_action(self.__term()))
            '''
            self.__scan()
            ...
            self.__check_rpar()           
            '''
        # TODO: need to parse expr like (maximize x), not only (maximize (+ x y))
        #  but when disabling check_lpar and check_rpar, ddsmt often fails for (maximize x);; why?
        elif self.la == SMTParser.MAXIMIZE:
            self.__scan()
            tokens.append(self.term.parse_action(self.__term()))
            '''
            self.__scan()
